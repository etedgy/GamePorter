...
- FitGirl-repack finding (2026-07-06): `setup.exe` crashes with Wine core assertion `alloc_pages_vprot` (virtual.c:1032) during decompression, BEFORE any files written. Tested — fires identically with/without LARGE_ADDRESS_AWARE and regardless of fd limit; it's a Wine-core limit that hits CrossOver too, NOT fixable via our env. Workarounds: official/non-repack installer, or install on Parallels (user has it) + copy game folder into bottle drive_c. User owns THPS 1+2 on Xbox (legit), couldn't get Mac version. Our wine reports wine-7.7/GPTK 1.1 string.
